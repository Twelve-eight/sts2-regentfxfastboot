...
## 2026-09-16 (RFX-4 后续) 弹窗的三个真实缺陷:悬挂模态槽位,共享帧计数器,VDF 措辞

本节取代上一节里"载荷 `5afbf4b85e143c32` / 57856 B"那句证据 - 那之后又修了三个缺陷,
**当前载荷以本节为准**.AGENTS.md Sec 9:不复制会漂移的断言,以权威输出为准.

### 缺陷 1(高):悬挂的 `OpenModal` 会让弹窗永久静默 - 且恰好命中新订阅者

`ModNotice.TryShow` 原先用 `container.OpenModal != null` 判断槽位是否被占.该判断与
"有**存活**模态占着槽位"并不等价:

- `NConfirmModLoadingPopup.OnYesButtonPressed/OnNoButtonPressed` 与
  `NGenericPopup.OnYesButtonPressed/OnNoButtonPressed` 都以 `QueueFreeSafely()` 结束,**不调 `Clear()`**;
- `NModalContainer` 没有任何子节点离树回调(grep 过 `ChildExitingTree`/`_Notification` 等,零命中),
  所以 `OpenModal` 会持续指向一个已释放对象;
- **引擎自己也无法恢复**:`Add()` 在 `OpenModal != null` 时直接 early-return.这一点由本机
  建模验证明确证实(见下,inv3),也正是它让"交给引擎自己恢复"的想法不成立.

可达路径恰好是最要紧的那一次启动:`NConfirmModLoadingPopup` 的条件是
`SettingsSave.ModSettings == null && ModManager.Mods.Count > 0`(`NMainMenu.cs:484-486`),
即**首次订阅者**;答完它槽位就悬空一整个会话.此时弹窗会每次启动都静默失败,
重试预算耗尽后还会把它报告成"槽位一直忙" - 一个与事实不符的解释.

修法:`SlotBusy(container, kind)` 按**存活**判定.存活持有者只读不动;证明已释放的持有者用引擎
自己的 `Clear()` 释放一次(不递归,抛异常则报 busy 交回重试).`Clear()` 会释放所有非 backstop 子节点,
此处安全依据是引擎自身不变式:`Add()` 在 `OpenModal != null` 时拒绝,所以**最多只有一个存活的
非 backstop 子节点**,而记录在案的那个已释放.

**验证方式(诚实声明)**:这是逻辑验证,不是实机验证.本机没有该引擎的可运行测试宿主,
所以用 Python 建模 `NModalContainer` 的 `Add`/`Clear` 语义 + 悬挂持有者,断言决策表与不变式:

| 场景 | 期望 | 实测 |
|---|---|---|
| 空槽位 | shown | shown |
| 存活引擎模态占位 | busy(且不动它) | busy,持有者与子节点列表原样保留 |
| 悬挂的已释放模态 | shown(先释放) | shown,日志记录释放 |
| 悬挂 + `Clear()` 抛异常 | busy(不崩,不死循环) | busy |

不变式 inv3 是本次修复的依据:`Add()` 对悬挂持有者**也返回 false** -> 引擎不会自愈,
这段代码是唯一的恢复点.

### 缺陷 2(中):共享帧计数器让成功弹窗的预算判断说谎

`ModNoticeWatcher._Process` 原先在**每帧**都推进 `_frames` 并与 `MenuWaitFrameLimit=3600` 比较.
但成功弹窗的观察节点是在预热**结束后**创建的 - 本机实测(BootTimer,39 mod):主菜单约 **4.0s** 出现,
预热跑在 4-22s,所以"创建时主菜单已在"是成功路径的**常态**.于是计数器在恰好第 120 次尝试那一帧
到达上限,顶部检查先命中:

- 丢弃日志会说"主菜单在 3600 帧内没有出现",而它已经出现了一分钟 - **假的解释**;
- `ShowAttemptLimit=120` 那段(DEVELOP.md 里写着的重试契约)**永远不会运行**.

修法:第一段预算只在 `!_menuSeen` 时计数,主菜单出现后冻结;第二段(120 次 x 30 帧,同样约 60s)
才是真正生效的上限.两段不再共享计数器.

### 缺陷 3(中):VDF 措辞与订阅者实际经历不符

- changenote 原文写"0.3.0 的失败弹窗未改动".但线上 changelog 只有 3 条、最新是 9-15 的 v0.2.0,
  `check-live-payload.ps1` 也确认 live 仍是 0.2.0 - **0.3.0 从未发布**.订阅者实际是
  **0.2.0 -> 0.4.0 一步跳**,两个弹窗对他们**都是新的**.原措辞会让订阅者以为失败弹窗是旧东西.
  已改写为"订阅者直接从 0.2.0 跳过来,两个弹窗在此都是新的",并分别说明两个弹窗的行为.
- 描述首句原写"无论本次是否被加速,主菜单的一次性弹窗都会告诉你结果".但部分完成/有失败时
  (`CompleteWarmUp` 中 `Pending > 0` 或 `FailedPaths > 0`,例如玩家在队列排空前就进了对局)
  **不弹任何窗**,且每个弹窗一生只弹一次.已收紧为只承诺两种被覆盖的结局
  ("确定未加速"与"完整加速"),并明确说明静默不代表"什么都没发生",细节在日志里.
