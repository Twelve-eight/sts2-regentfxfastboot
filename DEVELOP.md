...
- **内嵌 sha 滞后是结构性不可收敛的,不要再试图消除**:把载荷提交进仓库这一步本身就会移动 HEAD,
  于是刚暂存的载荷立刻又"落后一提交".实测循环:`4afe28b` 暂存 -> 提交变 `27a0de2` -> 再暂存 -> 提交变 `3f04df8`.
  **判定标准因此是 IL 同一性,不是内嵌 sha 或哈希相等**:
  暂存载荷的 `-il` 输出与当前 HEAD 构建逐字符相同(372099 字符,零差异)即视为同一,
  内嵌 `1.0.0+<sha>` 指向哪个提交不影响代码.
